...
mod tests {
    ...
    fn skips_trivia() {
        ...
        assert_eq!(p.non_trivia_indices.len(), 4);
        assert_eq!(p.current(), SyntaxKind::PACKAGE_KW);
        assert_eq!(p.nth(1), SyntaxKind::IDENT);
        assert_eq!(p.nth(2), SyntaxKind::L_CURLY);
        assert_eq!(p.nth(3), SyntaxKind::R_CURLY);
        assert_eq!(p.nth(4), SyntaxKind::EOF);
    }
    ...
    fn bump_and_eat() {
        ...
        assert!(!p.eat(SyntaxKind::IDENT));
        assert_eq!(p.current(), SyntaxKind::L_CURLY);
    }
    ...
    fn marker_complete() {
        ...
        let m = p.start();
        p.bump(SyntaxKind::PACKAGE_KW);
        m.complete(&mut p, SyntaxKind::PACKAGE);
        let events = p.finish();
        assert!(matches!(
            events[0],
            Event::Start {
