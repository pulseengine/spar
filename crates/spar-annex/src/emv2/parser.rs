...
impl<'t> Parser<'t> {
    pub(crate) fn new(tokens: &'t [(Emv2Kind, usize)], source: &'t str) -> Self {
    ...
    pub(crate) fn current(&self) -> Emv2Kind {
        self.nth(0)
    }

    pub(crate) fn nth(&self, n: usize) -> Emv2Kind {
        let idx = self.pos + n;
    ...
    pub(crate) fn at(&self, kind: Emv2Kind) -> bool {
        self.current() == kind
    }
    ...
    pub(crate) fn bump_any(&mut self) {
        ...
        if kind != Emv2Kind::EOF {
            self.do_bump(kind);
        }
    }

    pub(crate) fn eat(&mut self, kind: Emv2Kind) -> bool {
        if self.at(kind) {
            self.do_bump(kind);
            true
        } else {
    ...
    pub(crate) fn expect(&mut self, kind: Emv2Kind) -> bool {
        if self.eat(kind) {
            true
        } else {
    ...
    pub(crate) fn err_and_bump(&mut self, msg: &str) {
        ...
        self.error(msg.to_string());
        self.bump_any();
        m.complete(self, Emv2Kind::ERROR);
    }

    fn do_bump(&mut self, kind: Emv2Kind) {
