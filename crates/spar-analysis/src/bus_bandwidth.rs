        ...
        for &(bus_idx, _bus_name) in &buses {
            ...
            if total_demand_bps > capacity_bps {
                ...
                diags.push(AnalysisDiagnostic {
                    ...
                    message: format!(
                        "bus '{}' bandwidth utilization is high: {:.1} bps demand vs {:.1} bps \
                         capacity ({:.1}% utilization, {} bound connections)",
                        bus_comp.name,
                        total_demand_bps,
                        capacity_bps,
                        utilization,
                ...
                diags.push(AnalysisDiagnostic {
                    ...
                    message: format!(
                        "bus '{}' bandwidth utilization: {:.1} bps of {:.1} bps \
                         ({:.1}%, {} bound connections)",
                        bus_comp.name,
                        total_demand_bps,
                        capacity_bps,
...
mod tests {
    ...
    fn build_basic_model(
        ...
        b.set_children(receiver, vec![receiver_thread]);
